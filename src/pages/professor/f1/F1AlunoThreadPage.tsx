...
/**
 * Thread de observações de um aluno (Fundamental 1): estilo conversa.
...
const F1AlunoThreadPage = () => {
  ...
  // A fase vem da TURMA DESTE ALUNO (não do contexto, que só vê a 1ª turma do professor)
  const { data: faseTurma } = useFaseTurma(thread?.turmaId, profile?.institution_id, 'fundamental1');
  ...
  // O avulso NÃO exige fase ativa: antes do módulo 1 e depois do 8,
  // o registro segue vivo: grava com fase_id null ("não havia módulo ativo").
          ...
          <div className="space-y-4">
            {grupos.map((grupo, gi) => (
              <div key={gi} className="space-y-2">
                {/* Separador de módulo: pílula central estilo "balão de data" */}
                <div className="flex items-center gap-2 my-3">
                  ...
                  <span
                    ...
                    style={{ backgroundColor: t.accentSoft, color: t.accentText }}
                  >
                    {grupo.faseNome === 'Registro avulso' ? grupo.faseNome : `Módulo ${nomeModulo(grupo.faseNome)}`}
                  </span>
                  <div className="flex-1 h-px" style={{ backgroundColor: t.border }} />
                </div>

                {grupo.itens.map((obs, oi) => {
                  // Separador de dia dentro do módulo: um módulo dura semanas;
        ...
        >
          ...
          >
            ...
            <div className="p-5 space-y-3">
              ...
              <p className="text-sm leading-relaxed" style={{ color: t.textMuted }}>
                A maquete, o desenho, a coleção: o que as mãos dele fizeram. O rosto do
...
export default F1AlunoThreadPage;
