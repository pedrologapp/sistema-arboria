...
/**
 * Aba INTELIGÊNCIAS (Fundamental 1): o SANTUÁRIO DO MECANISMO.
 *
 * Mesmo santuário do Infantil (conteúdo compartilhado do tópico 3.2 do mestre),
 * com uma linha discreta ligando cada inteligência ao nome de ação que o
 * módulo tem na Academia do F1. A professora PODE ver o nome técnico; quem
 * nunca vê são as crianças.
 */
const F1FasePage = () => {
  ...
  // da página continua lendo achando que é o mecanismo anterior
  useEffect(() => {
    ...
    <>
      ...
      <div className="relative z-10 pt-4 pb-6" key={atual} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        {/* Entrada da FORMAÇÃO ARBORIA: no topo, visível em todas as inteligências
            (a Formação é compartilhada entre segmentos) */}
          ...
          {Array.from({ length: TOTAL }).map((_, i) => {
            ...
            return (
              <button
                ...
                aria-label={`${SANTUARIO_INFANTIL[n].nome}${ehFaseAtual ? ' (módulo atual da turma)' : ''}`}
                aria-pressed={ehLido}
              >
                {n}
              </button>
        ...
        {/* Legenda dos dois destaques */}
        <p className="text-[10px] text-center mb-3 vf-rise" style={{ color: 'rgba(255,255,255,0.5)' }}>
          {faseAtualValida > 0
            ? 'Branco = você está lendo · Anel = o módulo da sua turma agora'
        ...
        {faseAtualValida > 0 && atual !== faseAtualValida && (
          <div className="flex justify-center mb-3 vf-rise">
            ...
            >
              <ChevronLeft size={13} /> Voltar ao módulo atual ({SANTUARIO_INFANTIL[faseAtualValida].nome})
            </button>
          </div>
        )}

        {/* Turma ainda em planejamento: diz explicitamente */}
        {faseAtualValida === 0 && (
        ...
        {/* UMA ATIVIDADE, OITO CAMINHOS: a peça que produz o "aha";
            entender é comparar. */}
        ...
        {/* A seção da lente: com a cor do mecanismo */}
        <div
