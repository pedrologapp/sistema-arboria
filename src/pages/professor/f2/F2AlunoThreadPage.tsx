...
// Motivos da exclusão (auditoria). Presets, não texto livre.
const MOTIVOS_EXCLUSAO = ['Aluno errado', 'Erro de digitação', 'Foto errada', 'Outro'] as const;

// Pergunta do composer RODA por dia (pergunta fixa vira gabarito)
...
/**
 * Thread de observações de um aluno (Fundamental 2, reforma): estilo conversa.
 * Cópia do thread do Infantil/F1 com a língua do F2: cada observação é uma
 * mensagem; agrupadas por Casa (fase). Campo livre embaixo = registro avulso.
 * A fase é lida da TURMA do aluno com o segmento fundamental2 (a Casa em curso).
 */
const F2AlunoThreadPage = () => {
  ...
  // A fase vem da TURMA DESTE ALUNO (não do contexto, que só vê a 1ª turma do
  // professor). Segmento fundamental2: resolve a Casa em curso da turma.
  const { data: faseTurma } = useFaseTurma(thread?.turmaId, profile?.institution_id, 'fundamental2');
  const faseAtual = faseTurma?.fase ?? null;

  const primeiroNome = (thread?.aluno?.nome || '').split(' ')[0] || 'o aluno';
  ...
  // Aviso da foto ANTES do seletor abrir (uma vez só)
  ...
  // O avulso NÃO exige fase ativa: antes da 1ª Casa e depois da última,
  // o registro segue vivo: grava com fase_id null ("não havia Casa ativa").
  ...
  // Agrupa por fase (Casa), preservando a ordem cronológica
  ...
  return (
    <div style={{ backgroundColor: t.bg, minHeight: '100vh' }}>
          ...
          <div className="space-y-4">
            {grupos.map((grupo, gi) => (
              <div key={gi} className="space-y-2">
                {/* Separador de Casa: pílula central estilo "balão de data" */}
                <div className="flex items-center gap-2 my-3">
                  ...
                  <span
                    ...
                    style={{ backgroundColor: t.accentSoft, color: t.accentText }}
                  >
                    {grupo.faseNome === 'Registro avulso' ? grupo.faseNome : `Casa ${grupo.faseNome}`}
                  </span>
                  <div className="flex-1 h-px" style={{ backgroundColor: t.border }} />
                </div>

                {grupo.itens.map((obs, oi) => {
                  // Separador de dia dentro da Casa: uma fase dura semanas;
      ...
      >
        {/* Input de arquivo escondido, sem `capture`: câmera OU galeria */}
        <input
          ref={fileRef}
          type="file"
          accept="image/*"
          ...
          >
            ...
            <div className="p-5 space-y-3">
              <h2 className="text-base font-bold leading-snug" style={{ color: t.text }}>
                Fotografe o trabalho, não o aluno.
              </h2>
              <p className="text-sm leading-relaxed" style={{ color: t.textMuted }}>
                A maquete, o desenho, a produção: o que as mãos dele fizeram. O rosto do
                aluno não entra no diário.
              </p>
              <button
...
export default F2AlunoThreadPage;
