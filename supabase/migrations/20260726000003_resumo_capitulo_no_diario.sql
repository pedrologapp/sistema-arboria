-- ============================================================
-- finalizar_fase_turma: ao finalizar a fase (F2), escreve no diario de cada aluno
-- o RESUMO do que ele fez no capitulo daquela fase (o tema/papel do time). Pedido
-- do Fundador 26/07: o Livro do aluno precisa puxar o que veio dos capitulos, nao
-- so as observacoes soltas.
--
-- ADITIVO: mantem 100% da logica existente de virada de fase; so acrescenta o
-- bloco do resumo (guardado a F2, so quando ha capitulo, com NOT EXISTS pra nao
-- duplicar em re-finalizacao). A observacao entra como origem_captura='capitulo'
-- (a etiqueta "Na apresentacao" ja mostra no diario).
-- ============================================================
...
BEGIN
  ...
  -- >>> NOVO: resumo do capitulo no diario (so F2). O que o aluno fez = o time
  --     (papel categoria 'time') e o grupo. Nao duplica em re-finalizacao.
  IF v_seg = 'fundamental2' THEN
