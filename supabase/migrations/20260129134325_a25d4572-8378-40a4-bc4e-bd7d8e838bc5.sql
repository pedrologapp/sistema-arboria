-- =====================================================
-- A) Criar tabela de log/auditoria do webhook N8N
-- =====================================================
...
-- Índice para buscar por observacao_id
CREATE INDEX idx_webhook_n8n_logs_observacao ON public.webhook_n8n_logs(observacao_id);

-- Índice para limpeza por data
CREATE INDEX idx_webhook_n8n_logs_created ON public.webhook_n8n_logs(created_at);

-- Habilitar RLS (sem políticas = privado, só admin/sistema acessa)
ALTER TABLE public.webhook_n8n_logs ENABLE ROW LEVEL SECURITY;

-- =====================================================
-- B) Atualizar função com fail-safe e logging
-- =====================================================
...
DECLARE
  ...
  v_payload JSONB;
  v_request_id BIGINT;
  v_endpoint_url TEXT := 'https://n8n.escolaamadeus.com/webhook-test/projetoarboria';
BEGIN
...
-- =====================================================
-- C) Função para limpeza automática (logs > 14 dias)
-- =====================================================
...
DECLARE
  v_deleted INTEGER;
BEGIN
  DELETE FROM public.webhook_n8n_logs
