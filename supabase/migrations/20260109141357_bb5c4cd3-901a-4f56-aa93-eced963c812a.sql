...
-- Corrigir função get_missoes_do_aluno para considerar missão destinatário manual como NÃO global
-- Se uma missão tem destinatários manuais (existe em missao_destinatarios), 
-- ela NÃO é global mesmo com casa_id=NULL e serie_filtro=NULL
...
BEGIN
  ...
  WHERE 
    ...
    -- Lógica de destinação (REGRA CRÍTICA CORRIGIDA)
    AND (
      -- GLOBAL: para todos da instituição (casa_id NULL, serie_filtro NULL, E SEM destinatários manuais)
      (
        m.casa_id IS NULL 
        AND m.serie_filtro IS NULL
