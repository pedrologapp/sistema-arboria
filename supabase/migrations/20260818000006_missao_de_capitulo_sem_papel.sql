-- ============================================================
-- MISSAO DE CAPITULO SEM PAPEL VOLTA A APARECER
--
-- get_missoes_do_aluno, no ramo das missoes de CAPITULO, exigia uma das duas:
--
--   m.papel_id IS NOT NULL  e o aluno alocado naquele papel
--   m.para_membros_delegacao = true  e o aluno na delegacao
--
-- As duas missoes da Arena ("Apresentem seu projeto" e "Missao Individual") tem
-- capitulo, mas papel_id NULL e para_membros_delegacao false. Ou seja, elas
-- nunca casavam com nenhum dos dois ramos e NUNCA apareceram na home nem na aba
-- Missoes de ninguem. Os alunos so chegavam nelas entrando no Capitulo, e foi
-- por isso que tanta gente disse que nao via missao: em 18/08, 106 dos 162
-- alunos do F2 nao enxergavam missao nenhuma.
--
-- Missao de capitulo sem papel e sem delegacao e' missao GERAL do capitulo: e'
-- de todo mundo da turma que teve o capitulo liberado. E o que este terceiro
-- ramo passa a dizer. As condicoes de turma liberada continuam valendo, entao
-- nenhuma missao vaza para turma que o mentor ainda nao liberou.
--
-- Para reverter: apagar apenas o ramo marcado com o comentario NOVO abaixo.
-- ============================================================
    ...
    AND (
      ...
      (
        ...
        AND (
          ...
          OR
          -- NOVO: geral do capitulo. Sem papel e sem delegacao = de todo mundo
          -- da turma liberada. Sem este ramo a missao existe e nao aparece.
          (m.papel_id IS NULL AND COALESCE(m.para_membros_delegacao, false) = false)
        )
      )
      OR
...
-- Confere no Theo (6o B, EmpireOS): tem que passar a ver 3.
select titulo, ja_entregou, atrasada
  from public.get_missoes_do_aluno('53d55517-d271-495f-9ef5-0d55aece105b');
