-- Função de fechamento de fase
-- Calcula Score_Fase e Score_Novo para cada aluno × inteligência
-- Aplica fórmula de inércia (85/15) e limite de ±5 pontos

CREATE OR REPLACE FUNCTION fechar_fase(p_fase_id uuid)
RETURNS TABLE(alunos_processados int, fase_numero smallint) AS $$
DECLARE
  v_fase record;
  v_aluno record;
  v_im record;
  ...
  LOOP
    ...
    FOR v_im IN SELECT id FROM inteligencias ORDER BY id LOOP
      ...
      DO UPDATE SET
        ...
        score_apos_formula = v_score_novo;
